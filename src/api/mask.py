...
async def mask(request: MaskRequest):
    ...
    match request.backendType:
        case BackendType.LLM:
            ...
            return {"original_text": request.text, "entities": llm_entities, "anonymized_text": llm_entities}
        case BackendType.NER:
            spacy_entities = ano.find_entities(request.text)
            return {"original_text": request.text, "entities": spacy_entities, "anonymized_text": spacy_entities}
        case BackendType.REG:
            regex_entities = reg_find_entities(request.text)
            return {"original_text": request.text, "entities": regex_entities, "anonymized_text": regex_entities}
        case _:
